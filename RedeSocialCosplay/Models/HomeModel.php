<?php
    ...
    class HomeModel{
		...
		public static function BuscarPostsFeed(){
			...
			$posts = [];
			$texto = "select posts.data, posts.post, usuarios.nome, posts.status,posts.id,posts.usuario_id 
			from posts left join usuarios on posts.usuario_id = usuarios.id 
			where (($where) and (posts.status = 1)) or (posts.usuario_id = ?)  order by posts.data DESC";
			if (\RedeSocialCosplay\Models\UsuariosModel::usuarioModerador()){
				$texto = "select posts.data, posts.post, usuarios.nome, posts.status,posts.id,posts.usuario_id  from posts left join usuarios on posts.usuario_id = usuarios.id order by posts.data DESC";
